...
| P3 | 组合优化 / 多资产配置 | 组合分析 MVP 已落地；真实长期使用优化待后续 | 在单票研究和观察池稳定后，支持组合层评分、仓位建议和风险汇总 |
...
- [x] `services/portfolio/portfolio_analyzer.py`：纯业务模块，启发式权重分配、行业/资产暴露、风险汇总
- [x] `services/portfolio/report_builder.py`：JSON + Markdown artifact 输出
- [x] `apps/api/routers/portfolio.py`：POST /api/v1/portfolio/analyze，支持观察池和显式持仓两种来源
- [x] `apps/dashboard/pages/5_组合分析.py`：Streamlit 工作台页面
- [x] 离线验收脚本 `scripts/verify_portfolio_analysis.py`
- [x] 单元测试 17 个 + API/RBAC contract 测试 12 个 + Dashboard contract 测试 7 个
- [x] 所有输出明确标注”研究建议，不构成交易指令”
...
真实长期使用优化待后续：多期组合回测、动态再平衡策略、风格因子暴露分析。
