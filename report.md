...
- **当前工作区补充修正**：2026-05-12 已修正 Celery Beat schedule 使用的任务名，使其匹配 worker 实际注册名；移除 Beat 对未监听 `beat` 队列的投递；新增 `tests/test_celery_schedule.py` 防止再次漂移；新增 `tests/integration/` opt-in live 集成测试基线；新增 `docs/verification.md` 和 `docs/integration_testing.md` 记录本地验证方法；新增默认关闭的 `WebNewsProvider`，用于国内新闻/政策/舆情事件增强，并在代码层强制新闻抓取禁用代理。2026-05-13 已将东方财富个股新闻设为优先源，并接入新浪财经滚动新闻、新华网财经即时新闻、华尔街见闻、第一财经、36氪、腾讯新闻、新浪热门、新浪新闻热门、澎湃、B站、抖音、CSDN、GitHub Trending、Google 热搜、微信读书热榜舆情和百度新闻 RSS 作为 fallback；新增 `tests/integration/test_web_news_network_live.py` 用于真实网络质量验收。
- **测试口径说明**：
...
- **研究能力仍有明显边界**：网页新闻/政策/舆情增强已有基础实现，但来源、去重和长期稳定性仍需扩展；Qlib、组合优化、历史回测、系统设置 UI 等尚未完成。
...
- 已新增 `services/data/providers/web_news_provider.py`，第一版使用东方财富个股新闻作为优先源，新浪财经滚动新闻、新华网财经即时新闻、华尔街见闻、第一财经、36氪、腾讯新闻、新浪热门、新浪新闻热门、澎湃、B站、抖音、CSDN、GitHub Trending、Google 热搜、微信读书热榜舆情和百度新闻 RSS 作为 fallback。
...
- 新闻抓取代码层强制禁用代理：清理常见代理环境变量，设置 `NO_PROXY=*`，并让 `requests.Session.trust_env=False`，避免用户开启 VPN 时影响东方财富、新浪财经、新华网、华尔街见闻、第一财经、腾讯新闻、新浪、澎湃、B站、抖音、百度新闻等国内接口。
- 参考本地 `tools/` 下的新浪财经、每日科技新闻、`NewsCrawler` 和 `hotToday` 仓库后，补充了新浪滚动新闻 fallback、新华网财经 `nodeart/list` fallback、多类热榜 fallback 和低质量新闻标题过滤；`NewsCrawler` 中的搜狐源暂未接入，原因是其门户新闻噪声较高，适合后续作为可选来源继续评估。
- `hotrank` 源只在标题或摘要命中公司名/股票代码时进入 `event_data`，避免 B站、抖音、GitHub、微信读书等泛热榜内容直接污染投研报告；当前实测中 36氪可能返回反爬挑战页、Google Trends 可能超时，均作为可隔离的可选子源处理。
- 已新增默认跳过的 `tests/integration/test_web_news_network_live.py`，需要 `RUN_WEB_NEWS_NETWORK=1` 才会真实请求新闻源；本地验收中东方财富 live smoke 已通过。

**影响**：

- 风险官和事件引擎已经可以消费公告以外的新闻事件，但第一版来源单一，不能视为完整舆情系统。
- 对政策、舆情、突发新闻敏感的标的，仍需增加多来源验证、时间过滤、去重质量和长期运行记录。
...
| 网页搜索服务 | ⚠️ 基础实现 | 部分实现 | 默认关闭的东方财富/新浪/新华网/华尔街见闻/第一财经/36氪/腾讯/新浪热门/澎湃/B站/抖音/CSDN/GitHub/Google/微信读书/百度新闻增强源已接入，并新增 opt-in 真实网络验收测试 |
| Qlib | ❌ 未完成 | 未实现 | 暂无接入 |
...
- 已补充 `RUN_WEB_NEWS_NETWORK=1` 的 opt-in 网络 smoke 测试，用于真实新闻源可用性验收。
...
> **核心功能已基本实现，工程化框架已搭建，真实数据源、新闻增强和运行态集成验证仍需持续补齐。**
...
4. 继续扩展网页搜索/新闻政策舆情服务的来源、去重质量和长期运行验收。
