...
**P2 第二阶段：真实历史回测落地**
目标：从“8 个离线构造样本”升级为“真实历史样本池”。

需要推进：
- 用 QMT/缓存/CSMAR/EVA 构建真实历史样本集，覆盖不同年份、行业、风格、牛熊震荡、极端下跌、财报窗口、重大事件窗口。
- 每个样本固定 `as_of`，记录之后 20/60/120 交易日收益、行业相对收益、最大回撤。
- 回测脚本输出评分分布、评级分布、建议动作分布、命中率、回撤保护效果、行业分位有效率。
- 对评分和保护器设验收阈值，例如高风险样本不得出现激进建议，placeholder/critical 必须触发保护，评分不能集中在单一区间。
- 将 artifact 从“样本通过/失败”升级为“研究质量报告”，能看出策略偏差。

验收标准：
- 至少 50-200 个真实历史样本。
- 覆盖股票、ETF、行业样本不足、亏损、缺财报、极端行情。
- 脚本可重复运行，输出稳定 JSON/Markdown。
- 修改评分、估值、保护器时能发现质量漂移。
