...
- **当前工作区补充修正**：2026-05-12 已修正 Celery Beat schedule 使用的任务名，使其匹配 worker 实际注册名；移除 Beat 对未监听 `beat` 队列的投递；新增 `tests/test_celery_schedule.py` 防止再次漂移；新增 `tests/integration/` opt-in live 集成测试基线；新增 `docs/verification.md` 和 `docs/integration_testing.md` 记录本地验证方法。
- **测试口径说明**：
