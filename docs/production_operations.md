...
| Celery Beat schedule | `storage/runtime/celerybeat-schedule` | `storage/prod/celerybeat-schedule` |
| Log output | Separate PowerShell windows, stdout only | `storage/logs/prod/*.log` files |
| PID management | None | `storage/prod/*.pid` files |
...
connectivity) and starts FastAPI, Celery worker, Celery Beat, and Streamlit
with background processes and PID files.
...
|---------|-------------|----------|----------|
| FastAPI | 8000 | `storage/logs/prod/api.log` | `storage/prod/api.pid` |
| Celery Worker | — | `storage/logs/prod/celery-worker.log` | `storage/prod/worker.pid` |
| Celery Beat | — | `storage/logs/prod/celery-beat.log` | `storage/prod/beat.pid` |
| Streamlit | 8501 | `storage/logs/prod/streamlit.log` | `storage/prod/streamlit.pid` |
...
| **Can clean** | `storage/prod/*.pid` (stale) | — | Cleaned by cleanup script |
...
`manifest.json` describing what was copied.
...
1. Stop all services.
2. Copy backup contents back to project root, preserving directory structure.
...
- Stale PID files (process no longer running).
...
2. Check the service log:
   ```powershell
   Get-Content storage\logs\prod\api.log -Tail 50
   Get-Content storage\logs\prod\celery-worker.log -Tail 50
   ```
...
1. Check worker and beat logs:
   ```powershell
   Get-Content storage\logs\prod\celery-worker.log -Tail 50
   Get-Content storage\logs\prod\celery-beat.log -Tail 50
   ```
...
2. If Beat schedule is corrupted, stop Beat and delete the schedule file:
   ...
   Remove-Item storage\prod\celerybeat-schedule -ErrorAction SilentlyContinue
