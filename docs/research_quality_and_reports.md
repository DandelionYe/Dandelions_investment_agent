...
| Phase 2 | 50+ 真实历史样本池 + 可重复质量报告 + 验收阈值 | ✅ 已完成：100 个 QMT + CSMAR/EVA/行业样本，严格验收通过 |
...
## P2 Phase 2B：真实 QMT 历史样本池（严格验收通过）
...
当前状态：100 个 `qmt_xtdata` 样本已生成，价格来源 100% QMT、基本面来源 100% EVA、估值来源 100% CSMAR/EVA、行业分位有效率 72%。严格 Phase 2B 验收通过。

**已知限制**：
- 行业库只有单一快照（2026-05-20），历史样本行业来源为 non-strict。
- 基本面只有 EVA partial（股本/BPS），缺少 ROE/利润增速等盈利指标。
- 评级/动作分桶有限（D/C 为主），因缺少盈利指标。
...
| 基本面/估值 | 手工构造 | EVA 股本/BPS + CSMAR PE/PB/PS/分位 + 行业同行分位 |
...
```json
{
  ...
  "fundamental_source": "local_csmar_eva_structure|missing",
  "valuation_source": "local_csmar_daily_derived|local_csmar_eva_structure|missing",
...
- `local_csmar_eva_structure`：CSMAR EVA 股本结构（total_volume/float_volume/market_cap/bps），严格 as_of
...
- **行业库只有单一快照**（2026-05-20）：所有 2021-2025 历史样本的行业来源标记为 `local_csmar_industry_non_strict`，不计入严格行业来源覆盖率（0%）。行业分位仍可从同行 PE/PB/PS 计算（72% 样本有效），但来源时间点不可严格验证。
- **基本面只有 EVA partial**：EVA 提供 total_volume/float_volume/market_cap/bps，不提供 ROE/gross_margin/net_profit_growth/revenue_growth。需要 QMT financial 才能补齐这些字段，但 Phase 2B 明确不接入 QMT financial。
