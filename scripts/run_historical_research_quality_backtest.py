...
def _build_markdown_report(summary: dict, fixture_path: str) -> str:
    """构建 Markdown 质量报告。"""
    lines: list[str] = []
    ...
    lines.append(f"- **placeholder 保护器命中率**: {summary['placeholder_guard_hit_rate']:.1%}")
    lines.append(f"- **critical 保护器命中率**: {summary['critical_guard_hit_rate']:.1%}")
    ...
    lines.append("")
    lines.append("| 评分分桶 | 样本数 | 平均20日收益 | 平均60日收益 | 平均20日最大回撤 |")
    lines.append("|---------|-------|------------|------------|---------------|")
    for bucket, data in sorted(summary.get("forward_return_by_score_bucket", {}).items()):
        lines.append(
            f"| {bucket} | {data['count']} | "
            f"{data['avg_return_20d']:.2%} | {data['avg_return_60d']:.2%} | "
            f"{data['avg_max_drawdown_20d']:.2%} |"
        )
    lines.append("")
    ...
    lines.append("")
    if summary["failed"] == 0:
        lines.append("所有历史样本均通过验收。评分/估值/保护器行为稳定。")
    else:
    ...
    lines.append("## 后续建议")
    lines.append("")
    lines.append("- 扩展样本覆盖：更多北交所、港股通、行业ETF")
    lines.append("- 接入真实 QMT 历史数据替换手动快照")
    lines.append("- 增加 120 日 forward return 分析")
...
def main() -> int:
    ...
    try:
        samples = load_historical_samples(sample_path)
    ...
    md_content = _build_markdown_report(summary, str(sample_path))
