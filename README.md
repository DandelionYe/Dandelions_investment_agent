...
> **注意**：测试脚本需要先启动 FastAPI 服务。健康检查返回 503 通常意味着 Redis 未启动——运行 `start_redis.ps1` 然后重启 uvicorn 即可。
...
**解决**：每次电脑重启后运行 `start_redis.ps1` 即可。该脚本会自动拉起 WSL 并启动 Redis 服务。
...
### API_Test.ps1 测试脚本报"无法连接到 API 服务"
