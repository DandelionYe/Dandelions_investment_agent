...
- 本地参考数据：CSMAR 行业分类、CSMAR 历史行业库、CSMAR 财务报表、EVA_Structure 股本/市值、个股日交易衍生指标快照库，用于行业同行池、严格 as_of 行业分位、盈利质量基本面、股本补齐、股息率、估值倍数和历史分位 fallback。
...
services/
  ...
  data/                      数据层（QMT/AKShare/mock + 本地 CSMAR/EVA provider + evidence schema + 新闻舆情 provider + 聚合器 + 缓存）
  research/                  研究引擎（评分 / 决策保护 / 基本面 / 估值 / 事件 / 历史质量回测）
  llm/                       DeepSeek 客户端
...
apps/
  ...
  api/                       FastAPI 网关（Celery + Redis + JWT + WS + 37 端点全保护）
    auth/
      ...
      rbac.py                多用户 owner 隔离与 admin 权限 helper
    routers/
...
CSMAR_INCOME_CSV=data/raw/csmar/financial_statements/Income Statement/FS_Comins.csv
CSMAR_CFD_CSV=data/raw/csmar/financial_statements/Cash Flow Statement (Direct Method)/FS_Comscfd.csv
CSMAR_CFI_CSV=data/raw/csmar/financial_statements/Cash Flow Statement (Indirect Method)/FS_Comscfi.csv
CSMAR_BALANCE_CSV=data/raw/csmar/financial_statements/Balance Sheet/FS_Combas.csv
CSMAR_DEBT_CSV=data/raw/csmar/financial_statements/Debt repayment capacity/FI_T1.csv
CSMAR_INDUSTRY_HISTORY_CSV=data/raw/csmar/industry_history/Basic Information Table/DEBT_INSTITUTIONINFO.csv
```
...
data/raw/csmar/financial_statements/
  Income Statement/FS_Comins.csv                  # 利润表
  Cash Flow Statement (Direct Method)/FS_Comscfd.csv
  Cash Flow Statement (Indirect Method)/FS_Comscfi.csv
  Balance Sheet/FS_Combas.csv                     # 资产负债表
  Debt repayment capacity/FI_T1.csv               # 资产负债率等补充字段
data/raw/csmar/industry_history/
  Basic Information Table/DEBT_INSTITUTIONINFO.csv # 历史行业分类
...
- `LocalCSMARFinancialStatementProvider`：直接读取本地财务报表 CSV，按严格 as_of 可见性计算 `revenue_ttm`、`net_profit_ttm`、`roe`、`gross_margin`、`net_margin`、`revenue_growth`、`net_profit_growth`、`debt_ratio`、`operating_cashflow_quality`。
- `LocalCSMARIndustryHistoryProvider`：直接读取历史行业 CSV，按 `EndDate <= as_of` 选择可见行业记录，2021-2022 优先 P0207，2023+ 优先 P0221，用于真实历史回测和严格行业分位。
...
生产/准生产环境建议使用 `scripts/prod/` 下的脚本，而不是开发启动脚本。完整说明见 [docs/production_operations.md](docs/production_operations.md)，配置模板见 `.env.production.example`。

```powershell
# 启动全部服务，含前置检查、PID 文件和日志管理
...
必须备份的运行时路径包括 `.env`、`storage/tasks.db`、`storage/watchlist.json`、`storage/cache/research_data.sqlite`、`storage/reference/`、`storage/reports/` 和 `storage/artifacts/`。可清理内容包括 `__pycache__/`、`.pytest_cache/`、`.ruff_cache/`、过期生产日志和陈旧 PID 文件。
...
| 端点 | 说明 |
|------|------|
| `ws://host:8000/ws/task/{task_id}` | 单票研究任务实时进度（owner 或 admin 可订阅） |
| `ws://host:8000/ws/batch/{batch_id}` | 观察池批量扫描实时进度（owner 或 admin 可订阅） |
| `ws://host:8000/ws/events` | 全局事件流（仅 admin 可订阅） |
...
| `PATCH /api/v1/auth/users/{user_id}` | admin | 启用/禁用用户、调整角色 |

默认管理员：`admin`（通过 `AUTH_ADMIN_USER` / `AUTH_ADMIN_PASS` 环境变量配置，首次使用请在 `.env` 中设置强密码）。
...
系统已在 JWT 基础上补齐多用户隔离。普通用户只能访问自己创建的研究任务、报告、观察池文件夹、观察项、标签和批次；管理员可跨用户查看和管理资源，并可通过用户管理接口调整账户状态和角色。详细权限矩阵见 [docs/rbac_multi_user.md](docs/rbac_multi_user.md)。

资源归属字段：

- `research_tasks.created_by`：研究任务 owner，报告下载通过 task owner 间接授权。
- `watchlist_folders.owner_username`、`watchlist_items.owner_username`、`watchlist_tags.owner_username`、`watchlist_batches.owner_username`：观察池相关资源 owner。
- WebSocket `/ws/task/{task_id}` 和 `/ws/batch/{batch_id}` 按 owner 校验；`/ws/events` 仅管理员可订阅。
- Streamlit 报告库改为通过 API 获取报告，不再直接扫描 `storage/reports`；观察池本地 SQLite fallback 默认关闭，仅在 `STREAMLIT_LOCAL_STORE_FALLBACK=true` 时启用。
- 旧数据迁移归属到 `'default'` owner；默认单机自用模式仍可直接使用 admin 账户。
...
当前研究质量体系已经从离线样本扩展到真实历史样本和字段级证据链。详细说明见 [docs/data_quality_regression.md](docs/data_quality_regression.md) 与 [docs/research_quality_and_reports.md](docs/research_quality_and_reports.md)。

### P0 数据质量回归样本

`tests/fixtures/data_quality_regression_samples.json` 固定了一组真实数据质量回归样本，覆盖大盘完整缓存、EVA 股本 fallback、CSMAR 日衍生 fallback、亏损/缺基本面、行业同行不足或预检失败等路径。默认契约测试不依赖 QMT、网络、Redis 或 SQLite：

```powershell
python -m pytest tests/test_data_quality_regression_contract.py -q
...
python -m pytest tests/integration/test_data_quality_regression_live.py -q -s
```
...
Phase 2B 已落地 100 个真实 QMT 历史样本，范围为 2021-2025 沪深主板 A 股和用户指定边界样本，基准为沪深300（`000300.SH`）。样本使用 MiniQMT 日 K 线计算 20/60/120 交易日前瞻收益、相对基准收益和最大回撤，并按严格 as_of 接入本地 CSMAR 财务报表、CSMAR Daily Derived、EVA 股本/BPS 和历史行业库。

```powershell
# 需要 MiniQMT 已启动并登录
python scripts/build_historical_research_samples.py --use-qmt --require-qmt --asset-scope mainboard-a --start-year 2021 --end-year 2026 --benchmark 000300.SH --min-samples 50 --overwrite
...
当前严格验收基线：基本面来源覆盖率 100%，行业来源覆盖率 99%，估值来源覆盖率 72%，完整研究输入覆盖率 71%，严格行业分位有效率 70.71%，评级/动作分桶数均为 3。artifact 输出到：
...
### Evidence Schema 全链路化

`services/data/evidence_schema.py` 提供统一字段证据结构：
...
`services/data/news_quality.py` 提供网页新闻/舆情去重、相关性评分、质量分类、provider 失败降级和汇总。离线样本覆盖相关新闻、重复 URL/标题、低质量推广、不相关热榜、provider 不可用和超时：
...
`services/data/news_quality_monitor.py` 对 10 个核心 A 股标的运行真实 WebNewsProvider 抓取，按 provider/source 维度评估长期稳定性：
...
# 真实网络模式
python scripts/run_web_news_quality_monitor.py --sources eastmoney,sina --limit 5 --timeout-seconds 8
```

输出：

- `storage/artifacts/web_news_quality/live/latest.json`
- `storage/artifacts/web_news_quality/live/latest.md`
- `storage/artifacts/web_news_quality/live/history.jsonl`
...
python -m pytest tests/test_data_quality_regression_contract.py -q     # P0 真实数据质量回归契约
python -m pytest tests/test_production_operations_contract.py -q       # 生产运维脚本静态契约
...
python -m pytest tests/integration/test_web_news_network_live.py tests/integration/test_web_news_quality_monitor_live.py -q -p no:cacheprovider
```

真实数据质量回归和历史 QMT 回测为 opt-in，要求 MiniQMT 和本地参考数据可用：

```powershell
$env:RUN_DATA_QUALITY_REGRESSION='1'
python -m pytest tests/integration/test_data_quality_regression_live.py -q -s

$env:RUN_HISTORICAL_QMT_BACKTEST='1'
...
- 单票研究基本面仍可读取 MiniQMT 可见的本地财务表 `Balance`、`Income`、`CashFlow`、`PershareIndex`；真实历史回测样本池使用本地 CSMAR 财务报表 CSV，并按严格 as_of 可见性计算盈利质量指标。
- 行业分类和同行池默认来自本地 CSMAR `TRD_Co.csv` 构建的 `csmar_industry.sqlite`，不再默认使用 QMT sector。
- 历史行业分类优先来自 `data/raw/csmar/industry_history/Basic Information Table/DEBT_INSTITUTIONINFO.csv`，按 `EndDate <= as_of` 选择历史可见记录；无法严格验证时标记为 non-strict，不伪装成 strict source。
...
- 真实历史回测已接入本地 CSMAR 财务报表和历史行业库，100 个 QMT 历史样本的严格 Phase 2B 验收已通过。
- 关键研究字段已接入 `evidence_fields`，可审计 `value/source/as_of/quality/warnings`；报告和 LLM compact context 只展示摘要。
