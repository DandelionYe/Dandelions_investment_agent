...
st.set_page_config(page_title="单票研究", page_icon="🔬", layout="wide")
require_login()
...
def render_hitl_review():
    ...
    selected_action = st.selectbox(
        "覆盖最终操作建议", options=action_options, index=0,
        help="选择「不修改」则保留 AI 原始建议；选择其他值将覆盖。",
    )
    reviewer_notes = st.text_area(
        "审核备注", value="", placeholder="可在此记录审核意见、修改理由等……", height=80,
    )
...
def render_downloads(result: dict):
    ...
    if paths.get("pdf") and Path(paths["pdf"]).exists():
        with col1:
            st.download_button(
                label="下载 PDF 报告", data=read_file_bytes(paths["pdf"]),
                file_name=Path(paths["pdf"]).name, mime="application/pdf",
            )
    if paths.get("markdown") and Path(paths["markdown"]).exists():
        with col2:
            st.download_button(
                label="下载 Markdown", data=read_file_text(paths["markdown"]),
                file_name=Path(paths["markdown"]).name, mime="text/markdown",
            )
    if paths.get("json") and Path(paths["json"]).exists():
        with col3:
            st.download_button(
                label="下载 JSON", data=read_file_text(paths["json"]),
                file_name=Path(paths["json"]).name, mime="application/json",
            )
    if paths.get("html") and Path(paths["html"]).exists():
        with col4:
            st.download_button(
                label="下载 HTML", data=read_file_text(paths["html"]),
                file_name=Path(paths["html"]).name, mime="text/html",
            )
...
st.title("单票研究")
st.caption("输入沪深京 A 股或 ETF 代码，生成量化评分 + LLM 辩论 + 决策报告")

with st.sidebar:
    st.header("研究参数")
    symbol = st.text_input(
        "股票 / ETF 代码", value="600519.SH",
        help="示例：600519.SH、000001.SZ、510300.SH",
    )
    data_source = st.selectbox(
        "数据源", options=["qmt", "akshare", "mock"], index=0,
            ...
            with st.spinner("正在执行多轮辩论，完成后将请您审核……"):
                try:
                    hitl_package = start_hitl_research(
                        symbol=symbol.strip(), data_source=data_source,
                    )
