...
async def create_folder(req: WatchlistFolderCreate):
    """创建文件夹。"""
...
async def update_folder(folder_id: str, req: WatchlistFolderUpdate):
    """更新文件夹。"""
    try:
...
async def add_item(req: WatchlistItemCreate):
    """添加标的到观察池。"""
    try:
...
async def update_item(item_id: str, req: WatchlistItemUpdate):
    """更新观察项。"""
    try:
        kwargs = {k: v for k, v in req.model_dump().items() if v is not None}
...
async def create_tag(req: WatchlistTagCreate):
    """创建标签。"""
    try:
...
async def update_tag(tag_id: str, req: WatchlistTagUpdate):
    """更新标签。"""
    try:
        kwargs = {k: v for k, v in req.model_dump().items() if v is not None}
...
async def get_scan_progress(batch_id: str):
    """查询批量扫描进度。"""
    try:
