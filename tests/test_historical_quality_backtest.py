...
def samples() -> list[dict]:
    return load_historical_samples(FIXTURE_PATH)
...
class TestValidateHistoricalSample:
    ...
    def test_valid_etf_passes(self, samples):
        etf = next(s for s in samples if s["asset_type"] == "etf")
        errors = validate_historical_sample(etf)
        assert errors == []
    ...
    def test_quality_must_be_complete(self, samples):
        ...
        errors = validate_historical_sample(bad)
        assert any("data_complete" in e for e in errors)
...
class TestEvaluateHistoricalSample:
    ...
    def test_etf_sample_evaluates(self, samples):
        etf = next(s for s in samples if s["asset_type"] == "etf")
        result = evaluate_historical_sample(etf)
        assert "score" in result
    ...
    def test_high_quality_low_valuation(self, samples):
...
class TestSummarizeHistoricalBacktest:
    ...
    def test_scenario_coverage(self, summary):
...
class TestAssertHistoricalAcceptance:
    ...
    def test_acceptance_passes(self, summary):
        """默认阈值下应通过。"""
